const SCREENS=[
 {src:'./assets/mockups/01-intro.webp',label:'CASE 소개'},
 {src:'./assets/mockups/02-clues.webp',label:'단서 탐색'},
 {src:'./assets/mockups/03-chat.webp',label:'상담 대화'},
 {src:'./assets/mockups/04-board.webp',label:'시그널 분석'},
 {src:'./assets/mockups/05-report.webp',label:'AI 리포트 검토'},
 {src:'./assets/mockups/06-twist.webp',label:'새로운 메시지'},
 {src:'./assets/mockups/07-plan.webp',label:'상담 계획'},
 {src:'./assets/mockups/08-result.webp',label:'결과 확인'}
];
...
function renderIntro(stage){
 add(stage,{x:61,y:70,w:34,h:12,label:'미션 시작하기',cls:'pulse',action:()=>go(1)});
}
function renderClues(stage){
 const items=[
  ['door',18,27,7,11,'교실 표지판 단서'],['friends',31,44,12,15,'친구들의 대화 단서'],['notice',17,61,10,14,'상담 안내문 단서'],['phone',60,42,9,14,'민서의 휴대폰 단서'],['desk',18,68,10,13,'책상 주변 단서']
 ];
...
function renderChat(stage){
 const choices=[
  {x:42,y:68,w:16,h:14,label:'속상한 것 같아요',msg:'감정을 단정하기보다 더 물어보는 게 좋아.'},
  {x:59,y:68,w:16,h:14,label:'좀 답답해요',msg:'좋아. 민서가 말하기 편한 표현을 골랐어.'},
  {x:76,y:68,w:16,h:14,label:'그냥 복잡해요',msg:'좋아. 여러 감정이 섞일 수 있다는 걸 열어뒀어.'}
 ];
...
function renderBoard(stage){
 const cards=[
  [18,35,23,12,'중요한 시그널 카드'],[18,50,23,12,'중요한 시그널 카드'],[45,35,22,12,'확인이 필요한 카드'],[48,65,19,11,'주말 약속 취소 카드'],[72,37,20,12,'추측 카드'],[72,52,20,12,'추측 카드']
 ];
 ...
 add(stage,{x:61,y:87,w:15,h:9,label:'다음 AI 리포트 검토 단계',action:()=>go(4)});
...
function renderReport(stage){
 ...
 options.forEach(([id,x,y,w,h,label])=>add(stage,{x,y,w,h,label,cls:state.report.has(id)?'selected':'',action:(e,b)=>{state.report.add(id);document.querySelectorAll('.hit[data-judge]').forEach(x=>x.classList.remove('selected'));b.dataset.judge='1';b.classList.add('selected');showToast(label==='더 알아야 해요'?'맞아. 사람의 성향을 단정하려면 정보가 더 필요해.':'AI의 문장도 근거를 확인하며 읽어야 해.');setTimeout(()=>go(5),900)}}));
}
function renderTwist(stage){
 add(stage,{x:62,y:73,w:33,h:11,label:'새 메시지 반영하기',cls:'pulse',action:()=>{showToast('새 정보를 반영했어. 처음 판단을 수정해도 괜찮아.');setTimeout(()=>go(6),650)}});
}
function renderPlan(stage){
 const cards=[
  [15,28,12,19,'더 많이 들어주기'],[28,28,12,19,'마음 인정하기'],[41,28,12,19,'학교생활 살펴보기'],[15,50,12,19,'선생님과 이야기하기'],[28,50,12,19,'친구 관계 살펴보기'],[41,50,12,19,'전문기관 연결하기']
 ];
...
function render(){
 ...
 const img=document.createElement('img');img.src=info.src;img.alt=`마음 SIGNAL LAB ${info.label} 시안`;img.draggable=false;stage.appendChild(img);
