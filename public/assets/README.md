...
- **`char/friends-full` 은 원본이 정강이에서 잘려 발이 없습니다.** 복도에
  세우면 바닥에 파묻힌 것처럼 보여서, 아랫부분을 마스크로 흐리게 날려
  햇빛에 번진 것처럼 처리했습니다 (`CaseScene.module.css` 의 `.faded`).
  발까지 나온 그림으로 바꾸면 `lib/cases/case01.js` 의 `fade: true` 를
  지우면 됩니다. 재생성 프롬프트는 PROMPTS.md 2-4 참고.
...
| `friends-full` | 친구 3명 그룹 — 대화 중 전신 (복도 장면 배경) | ⚠️ 발이 잘려 있음 |
