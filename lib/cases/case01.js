...
const CASE_01 = {
  ...
  scene: {
    ...
    figures: [
      /*
       * aspectRatio 는 반드시 원본 파일 비율과 같아야 한다. 다르면 contain 이
       * 좁은 쪽에 맞춰 축소해서, 발 위치는 그대로인데 키만 줄어든다.
       *
       * 키는 소실점(y≈50%)에서 발까지의 거리에 비례한다.
       * 민서는 발이 y≈99% 에 있고 키가 74% — 비례상수 약 1.51.
       *
       * friends-full 은 원본이 정강이에서 잘려 발이 없다(→ fade). 발이 닿을
       * 자리를 y≈88% 로 잡으면 온몸 키는 (88-50)*1.51 ≈ 57%, 그중 그림에
       * 남아 있는 부분이 약 9할이라 52%.
       */
      {
        id: "friends",
        ...
        ground: true,
        fade: true,
        style: { left: "7%", bottom: "12%", height: "52%", aspectRatio: "784 / 900" },
      },
      {
        id: "minseo",
  ...
  report: {
    ...
    lines: [
      ...
      {
        id: "r3",
        text: "친구들 쪽으로 몸을 돌리지 않은 것으로 보아 마음이 불편했을 수 있습니다.",
        answer: "inference",
